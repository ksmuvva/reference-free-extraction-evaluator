        ...
        return EvaluationEvidence(
            ...
            output_claims=[
                ClaimDecision(
                    ...
                    entity_correct=True,
                    contradiction=False,
                    matched_source_fact_ids=[source_facts[0].fact_id],
                    evidence_questions=[1],
                )
                for c in claims
            ],
            source_facts=[
                SourceFactDecision(
                    fact_id=source_facts[0].fact_id,
                    fact=source_facts[0].fact,
                    captured=True,
                    evidence_questions=[1],
                )
            ],
...
class EmptyNarrativeDecomposerJudge(FakeJudge):
    def decompose(self, field: str, value: str):
        return []


VALID_OUTPUT = {
    "reason_for_claim": "Supported fact",
    "medical_conditions": ["Supported fact"],
    "family_relationships": "Supported fact",
    "countries_travelled": ["Supported fact"],
}
...
def test_judge_contract_failure_becomes_indeterminate():
    ...
    assert any(
        "contract violation" in issue.lower()
        for issue in report.evidence[0].issues
    )


def test_zero_claim_narrative_decomposition_has_distinct_failure_message():
    report = evaluate_transcript(
        {"interview": []},
        VALID_OUTPUT,
        EmptyNarrativeDecomposerJudge(),
    )

    first = report.evidence[0]
    assert report.metrics.fields[0].status == "FAIL"
    assert any("zero atomic claims" in issue.lower() for issue in first.issues)
    assert all("output field is empty" not in issue.lower() for issue in first.issues)
